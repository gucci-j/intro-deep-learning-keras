...
model.save_weights('before_param.h5')
...
model.save_weights('after_param.h5')

plt.figure()
plt.plot(range(1, 20+1), loss, marker='.', label='train')
plt.plot(range(1, 20+1), val_loss, marker='.', label='test')
...
plt.clf()
plt.plot(range(1, 20+1), acc, marker='.', label='train')
plt.plot(range(1, 20+1), val_acc, marker='.', label='test')
plt.legend(loc='best', fontsize=10)
plt.grid()
plt.xlabel('Epochs')
plt.ylabel('Accuracy')
