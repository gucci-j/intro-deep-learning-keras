...
def set_flag():
    # Early Stoppingフラグの定義
    flag = {}
    flag['With EarlyStopping'] = True
    flag['Without EarlyStopping'] = False
...
def main():
    ...
    for key in flag.keys():
        ...
        if flag[key] is True:
            ...
            results[key] = model.fit(x=x_train, y=y_train, batch_size=batch_size, epochs=epochs, 
    ...
    for key in flag.keys():
        ...
        plt.plot(range(1, len(acc)+1), acc, marker='.', label='train')
        plt.plot(range(1, len(val_acc)+1), val_acc, marker='.', label='test')
    ...
    for key in flag.keys():
        ...
        plt.plot(range(1, len(loss)+1), loss, marker='.', label='train')
        plt.plot(range(1, len(val_loss)+1), val_loss, marker='.', label='test')
        plt.legend(loc='best', fontsize=10)
        plt.grid()
        plt.xlabel('Epochs')
        plt.ylabel('Loss')
